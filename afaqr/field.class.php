...
class profile_field_afaqr extends profile_field_base {
    ...
    public function display_data() {
		...
		$urlencoded = urlencode('otpauth://totp/'.$site->fullname.'-'.$current_user->firstname.'-'.$current_user->lastname.'?secret='.$data.'');
        $src = 'https://chart.googleapis.com/chart?chs=200x200&chld=M|0&cht=qr&chl='.$urlencoded.'';

        $img =  '<img src="'.$src.'"/>';
		$userinfo = $site->fullname.":".$current_user->firstname."-".$current_user->lastname;
